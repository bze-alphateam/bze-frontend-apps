...
export const RewardsStakingBox = ({stakingReward, onClick, userStake, userUnlocking} : RewardsStakingBoxProps) => {
    ...
    const isLoading = !stakingReward || stakingAssetIsLoading || prizeAssetIsLoading
    ...
    // Plain computations (no useMemo): they depend on getPoolByLpDenom, whose identity
    // the React Compiler can't guarantee, and the values are cheap string pairs.
    const resolveLpDenoms = (isLp: boolean, denom?: string) => {
        if (!isLp || !denom) return { base: '', quote: '' }
        const pool = getPoolByLpDenom(denom)
        return pool ? { base: pool.base, quote: pool.quote } : { base: '', quote: '' }
    }
    const stakingLpDenoms = resolveLpDenoms(isStakingLp, stakingReward?.staking_denom)
    const prizeLpDenoms = resolveLpDenoms(isPrizeLp, stakingReward?.prize_denom)
